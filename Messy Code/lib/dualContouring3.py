            ...
            for i in range(0,dims-1):
                ...
                if array[i,j,k] != array[i+1,j,k]:
                    ...
                    if normal == 'empty':
                        ...
                        print ([i,j,k],[i+1,j,k])
                        # check behind
                        stick_behind = createStick([i-5,j,k],[i,j,k],dims,scale,translate,split_obj)
                        print "stick behind"
                        intersection, normal = stick_behind.getHermite()
                        if isinstance(intersection[0],tuple):
                            print "SJALALAAA"
                        # check in front
                        stick_infront = createStick([i+1,j,k],[i+6,j,k],dims,scale,translate,split_obj)
                        print "stick in front"
                        intersection, normal = stick_infront.getHermite()
                        if isinstance(intersection[0],tuple):
                            print "SJALALAAA"
                        # CHECK PROBING
                    # add to dict
                ...
                if array[i,j,k] != array[i,j+1,k]:
                    ...
                    if normal == 'empty':
                        ...
                        print ([i,j,k],[i,j+1,k])
                        # check behind
                        stick_behind = createStick([i,j-5,k],[i,j,k],dims,scale,translate,split_obj)
                        print "stick behind"
                        intersection, normal = stick_behind.getHermite()
                        if isinstance(intersection[0],tuple):
                            print "SJALALAAA"
                        # check in front
                        stick_infront = createStick([i,j+1,k],[i,j+6,k],dims,scale,translate,split_obj)
                        print "stick in front"
                        intersection, normal = stick_infront.getHermite()
                        if isinstance(intersection[0],tuple):
                            print "SJALALAAA"
                ...
                if array[i,j,k] != array[i,j,k+1]:
                    ...
                    if normal == 'empty':
                        ...
                        print ([i,j,k],[i,j,k+1])
                        # check behind
                        stick_behind = createStick([i,j,k-5],[i,j,k],dims,scale,translate,split_obj)
                        print "stick behind"
                        intersection, normal = stick_behind.getHermite()
                        if isinstance(intersection[0],tuple):
                            print "SJALALAAA"                        
                        # check in front
                        stick_infront = createStick([i,j,k+1],[i,j,k+6],dims,scale,translate,split_obj)
                        print "stick in front"
                        intersection, normal = stick_infront.getHermite()
                        if isinstance(intersection[0],tuple):
                            print "SJALALAAA"
